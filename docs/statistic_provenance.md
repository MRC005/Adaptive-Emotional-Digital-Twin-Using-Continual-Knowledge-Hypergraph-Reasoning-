...
## 6. Where the two records live

Both are on disk, generated, and guarded by tests. Neither may overwrite the
other.

| | File | Generated by | Guard |
|---|---|---|---|
| **Originally reported** | `frontend/src/data/historical_findings.json` | `scripts/export_historical_baseline.py`, which reads git object `38f8785:frontend/src/data/findings.json` | `tests/regression/test_historical_baseline.py` asserts it still matches that object, that its twin figure is still 0.2847, and that the five baselines still agree with the regenerated file |
| **Regenerated** | `frontend/src/data/findings.json` | `scripts/export_findings.py` from the result files | `tests/regression/test_findings_provenance.py` |

The Research page reads both and shows them side by side under
"Reproducibility: two records, both kept". `README.md` separates them under
"As originally reported" and "As regenerated".

## 7. Reproducibility machinery for new experiments

Added 2026-09-05, so that the next discrepancy is answerable from the artefact
itself:

- `package_versions()` now records **scikit-learn, joblib and threadpoolctl**
  alongside numpy, scipy and pandas, under their install names. Their absence
  is why the first investigation had to reconstruct the environment from
  filesystem timestamps.
- `thread_environment()` records the OpenMP/MKL/OpenBLAS environment
  variables, the CPU count and every native thread pool with its version.
- `aedt/reporting/experiment_record.py` writes one `run_metadata.json` per run
  carrying the git commit, interpreter, package versions, threading, the
  dataset's own recorded SHA-256 digests, the full configuration and the seed.
